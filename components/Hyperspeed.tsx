// COMMENTED OUT - Not using hyperspeed transition anymore
/*
'use client';
...
const roadMarkings_fragment = `
  ...
  // Shoulder lines
  float shoulderLine = smoothstep(0., laneWidth * uShoulderLinesWidthPercentage, vUv.x) - smoothstep(laneWidth - laneWidth * uShoulderLinesWidthPercentage, laneWidth, vUv.x);
  color = mix(color, uShoulderLinesColor, shoulderLine);

  // Broken lines
  float brokenLine = smoothstep(0., laneWidth * uBrokenLinesWidthPercentage, vUv.x - laneWidth) - smoothstep(laneWidth - laneWidth * uBrokenLinesWidthPercentage, laneWidth, vUv.x - laneWidth);
  
  if (fract(vUv.y * 10. * uBrokenLinesLengthPercentage) > 0.5) {
...
class App {
  ...
  constructor(container: HTMLElement, options: HyperspeedOptions) {
    ...
    this.autoSpeedUp = options.isHyper || false;
  ...
  init() {
    ...
    // Auto-trigger speed up if specified
    if (this.autoSpeedUp) {
      setTimeout(() => {
        this.fovTarget = this.options.fovSpeedUp;
        this.speedUpTarget = this.options.speedUp;
      }, 200); // Small delay to let scene initialize
    }
